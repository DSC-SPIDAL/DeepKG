...
def main():
    print("\n" + "="*120)
    print(" 📊 DEEPCOLLECTOR: FINAL ACADEMIC ANALYZER")
    print("="*120)
            ...
            # --- Parse Logs for Time ---
                ...
                try:
                    ...
                    if len(chunks) == 1:
                        ...
                        if tm: time_data.append({'Project': proj, 'Configuration': config_name, 'Model_Base': model_base, 'Time': float(tm[-1])})
                    else:
                        for i in range(1, len(chunks), 2):
                            ...
                            if tm: time_data.append({'Project': chunk_proj, 'Configuration': config_name, 'Model_Base': model_base, 'Time': float(tm[-1])})
                except: pass
    ...
    if not df_time.empty:
        ...
        combined_time.name = 'Time_Str'  # Fixes the KeyError: 0
