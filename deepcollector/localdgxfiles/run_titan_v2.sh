#!/bin/bash
# DeepSeek 131K Titan: Isolated Re-run (With Run Tracking)
export DC_TEMP="0.6"
export DC_TOKENS="8192"

# Track run identifier for variance calculation
RUN_ID="run2" 
...
echo "========================================================"
echo "🚀 INITIALIZING TITAN ABLATION SUITE (DeepSeek 131K ONLY - ${RUN_ID})"
echo "========================================================"

for JOB in "${JOBS[@]}"; do
    read -r MODEL DATASET <<< "$JOB"

    LOG_FILE="Titan_${MODEL}_${DATASET}_128K_${RUN_ID}.log"
    VRAM_CSV="vram_${MODEL}_${DATASET}_${RUN_ID}.csv"

    echo -e "\n--------------------------------------------------------"
    echo "⚡ STARTING JOB: Model=$MODEL | Dataset=$DATASET | Run=$RUN_ID"
    echo "--------------------------------------------------------"
...
echo -e "\n========================================================"
echo "✅ DEEPSEEK TITAN RUNS COMPLETE (${RUN_ID})"
echo "========================================================"
