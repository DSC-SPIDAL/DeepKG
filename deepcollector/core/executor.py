# =============================================================================
# V1.1: Unified Executor Module (Thread Pool Cleanup Fix)
# =============================================================================
import os
import sys
import time
        ...
        if mode == "PLAN":
            ...
            for i, p_name in enumerate(project_names):
                ...
                if p_name != "GLOBAL":
                    ...
                    if loader.resolve_configuration():
                        ...
                        if current_config.PROJECT_METHOD == 2 and run_mode == "AGENT":
                            print(f"    ⚠️  Project defines Method 2 in Master Sheet. Switching to HARVEST mode.")
                            run_mode = "HARVEST"
                    else:
                ...
                try:
                    ...
                    # 🔥 CRITICAL FIX: Shut down the async thread pool so Python can safely exit!
                    if 'agent' in locals() and hasattr(agent, 'tools') and hasattr(agent.tools, 'thread_pool'):
                        agent.tools.thread_pool.shutdown(wait=False)
